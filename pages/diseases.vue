<template>
  <!-- Main -->
  <main class="main-dashboard">
    <!-- Content -->
    <section
      class="flex flex-row justify-between gap-6 flex-grow max-md:max-w-none"
    >
      <div class="flex flex-col gap-8 max-sm:gap-6 w-full">
        <!-- Headline -->
        <div class="flex justify-between">
          <h1 class="font-bold text-2xl text-dark self-center py-1">
            Disease List
          </h1>
          <button
            type="button"
            class="btn btn-primary p-3 self-center flex flex-row gap-1.5 items-center max-sm:p-[10px]"
            @click="showModal('formDisease', null)"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              stroke-width="2"
              stroke="currentColor"
              class="w-5 h-5"
            >
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
                d="M12 4.5v15m7.5-7.5h-15"
              />
            </svg>
            <p class="text-sm">Add Disease</p>
          </button>
        </div>

        <div class="flex gap-6 justify-between">
          <!-- Search -->
          <div class="flex justify-between w-full">
            <div class="relative w-full">
              <div
                class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none"
              >
                <svg
                  class="w-5 h-5 text-gray-500"
                  aria-hidden="true"
                  fill="currentColor"
                  viewBox="0 0 20 20"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    fill-rule="evenodd"
                    d="M8 4a4 4 0 100 8 4 4 0 000-8zM2 8a6 6 0 1110.89 3.476l4.817 4.817a1 1 0 01-1.414 1.414l-4.816-4.816A6 6 0 012 8z"
                    clip-rule="evenodd"
                  ></path>
                </svg>
              </div>
              <input
                type="text"
                id="table-search-diseases"
                class="block p-2 pl-10 text-sm text-gray-900 w-full h-full input-field"
                placeholder="Search for diseases"
                v-model="query.search"
                v-on:keyup.enter="$fetch()"
              />
            </div>
          </div>
        </div>

        <!-- Disease List -->
        <div class="flex flex-col gap-4 w-full">
          <!-- Show Content -->
          <div
            class="overflow-x-auto flex flex-wrap max-sm:flex-col gap-5 pb-4"
          >
            <div
              class="bg-white rounded-md overflow-clip p-0 flex-auto w-[250px] max-sm:w-full cursor-pointer shadow-sm hover:shadow-lg group flex flex-col justify-start"
              v-for="item in listDiseases"
              @click="showModal('formDisease', item.id)"
            >
              <!-- Disease -->
              <div class="w-full">
                <!-- Images -->
                <img
                  :src="item.image ? imgUrl + item.image : ''"
                  alt=""
                  class="w-full h-[150px] object-cover"
                  v-if="item.image"
                />

                <div
                  class="w-full h-[150px] flex justify-center items-center bg-gray-200"
                  v-else
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24"
                    fill="currentColor"
                    class="w-14 h-14 fill-gray-400"
                  >
                    <path
                      fill-rule="evenodd"
                      d="M1.5 6a2.25 2.25 0 012.25-2.25h16.5A2.25 2.25 0 0122.5 6v12a2.25 2.25 0 01-2.25 2.25H3.75A2.25 2.25 0 011.5 18V6zM3 16.06V18c0 .414.336.75.75.75h16.5A.75.75 0 0021 18v-1.94l-2.69-2.689a1.5 1.5 0 00-2.12 0l-.88.879.97.97a.75.75 0 11-1.06 1.06l-5.16-5.159a1.5 1.5 0 00-2.12 0L3 16.061zm10.125-7.81a1.125 1.125 0 112.25 0 1.125 1.125 0 01-2.25 0z"
                      clip-rule="evenodd"
                    />
                  </svg>
                </div>
              </div>

              <div class="flex flex-col gap-1 p-4 justify-between flex-grow">
                <div class="flex flex-col gap-1">
                  <!-- Name -->
                  <div class="text-lg text-gray-900 font-semibold">
                    {{ item.name }}
                  </div>

                  <!-- Description -->
                  <div class="text-sm text-gray-400 line-clamp-2">
                    {{ item.description }}
                  </div>

                  <!-- Tags -->
                  <div class="flex flex-wrap gap-2 items-start mt-2">
                    <div
                      class="text-sm text-gray-400 bg-gray-100 rounded-md py-1 px-2"
                      v-for="tag in item.tags"
                      v-if="tag.tag.tag"
                    >
                      {{ tag.tag.tag }}
                    </div>
                  </div>
                </div>

                <!-- Linked Article -->
                <div class="flex flex-col gap-1">
                  <div class="text-sm text-gray-600 mt-4">Linked Article:</div>

                  <div
                    class="flex gap-2 p-2 items-center border border-gray-200 rounded-md hover:border-gray-300"
                  >
                    <!-- Images -->
                    <img
                      :src="
                        item.article.article_images[0].image
                          ? imgUrl + item.article.article_images[0].image
                          : ''
                      "
                      alt=""
                      class="w-[40px] h-[40px] object-cover rounded-sm"
                      v-if="item.article.article_images[0]"
                    />

                    <div
                      class="w-[40px] h-[40px] flex justify-center items-center bg-gray-200 rounded-sm"
                      v-else
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 24 24"
                        fill="currentColor"
                        class="w-6 h-6 fill-gray-400"
                      >
                        <path
                          fill-rule="evenodd"
                          d="M1.5 6a2.25 2.25 0 012.25-2.25h16.5A2.25 2.25 0 0122.5 6v12a2.25 2.25 0 01-2.25 2.25H3.75A2.25 2.25 0 011.5 18V6zM3 16.06V18c0 .414.336.75.75.75h16.5A.75.75 0 0021 18v-1.94l-2.69-2.689a1.5 1.5 0 00-2.12 0l-.88.879.97.97a.75.75 0 11-1.06 1.06l-5.16-5.159a1.5 1.5 0 00-2.12 0L3 16.061zm10.125-7.81a1.125 1.125 0 112.25 0 1.125 1.125 0 01-2.25 0z"
                          clip-rule="evenodd"
                        />
                      </svg>
                    </div>

                    <!-- Title -->
                    <div
                      class="text-md text-gray-900 font-medium line-clamp-2 w-full"
                    >
                      {{ item.article.title }}
                    </div>

                    <!-- Edit Button -->
                    <button
                      type="button"
                      @click.stop
                      @click="showModal('formLinkedArticle', item.id)"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 20 20"
                        fill="currentColor"
                        class="w-5 h-5 fill-gray-300 hover:fill-gray-400"
                      >
                        <path
                          d="M5.433 13.917l1.262-3.155A4 4 0 017.58 9.42l6.92-6.918a2.121 2.121 0 013 3l-6.92 6.918c-.383.383-.84.685-1.343.886l-3.154 1.262a.5.5 0 01-.65-.65z"
                        />
                        <path
                          d="M3.5 5.75c0-.69.56-1.25 1.25-1.25H10A.75.75 0 0010 3H4.75A2.75 2.75 0 002 5.75v9.5A2.75 2.75 0 004.75 18h9.5A2.75 2.75 0 0017 15.25V10a.75.75 0 00-1.5 0v5.25c0 .69-.56 1.25-1.25 1.25h-9.5c-.69 0-1.25-.56-1.25-1.25v-9.5z"
                        />
                      </svg>
                    </button>
                  </div>
                </div>
              </div>
            </div>
          </div>

          <!-- Pagination -->
          <nav
            class="flex items-center justify-between pt-4"
            aria-label="Table navigation"
          >
            <span class="text-sm font-normal text-gray-500"
              >Showing
              <span class="font-semibold text-gray-900"
                >{{ diseases.from ? diseases.from : 0 }} -
                {{ diseases.to ? diseases.to : 0 }}</span
              >
              of
              <span class="font-semibold text-gray-900">{{
                diseases.total
              }}</span></span
            >
            <ul class="inline-flex items-center -space-x-px">
              <li class="cursor-pointer" v-for="page in listPages">
                <!-- Inactive -->
                <div
                  class="px-3 py-2 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700"
                  v-if="
                    !page.active &&
                    page.label != '&laquo; Previous' &&
                    page.label != 'Next &raquo;'
                  "
                  @click="changePage(page.label)"
                >
                  {{ page.label }}
                </div>

                <!-- Active -->
                <div
                  aria-current="page"
                  class="z-10 px-3 py-2 leading-tight text-white border border-primary-1 bg-primary-1 hover:bg-primary-2 hover:text-white"
                  v-if="page.active"
                >
                  {{ page.label }}
                </div>

                <!-- Previous -->
                <div
                  class="px-3 py-2 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 rounded-l-md"
                  v-if="!page.active && page.label == '&laquo; Previous'"
                  @click="previousPage()"
                >
                  &laquo;
                  <span class="max-sm:hidden"> Previous</span>
                </div>

                <!-- Next -->
                <div
                  class="px-3 py-2 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 rounded-r-md"
                  v-if="!page.active && page.label == 'Next &raquo;'"
                  @click="nextPage()"
                >
                  <span class="max-sm:hidden">Next </span>
                  &raquo;
                </div>
              </li>
            </ul>
          </nav>
        </div>
      </div>
    </section>

    <!-- Modal -->
    <div
      class="fixed inset-0 flex justify-center items-center bg-black bg-opacity-50 z-[999]"
      v-if="modal"
      @click="closeModal"
    >
      <div
        class="w-[600px] max-md:w-[90%] card p-4 bg-white overflow-y-auto shadow-lg h-[90%]"
        @click.stop
      >
        <FormDisease
          class=""
          :disease_id="disease_id"
          @close-form="closeModal"
          v-if="modalType == 'formDisease'"
        />
        <FormLinkedArticle
          class=""
          :disease_id="disease_id"
          @close-form="closeModal"
          v-if="modalType == 'formLinkedArticle'"
        />
      </div>
    </div>
  </main>
</template>

<script>
export default {
  middleware: 'admin',
  layout: 'LayoutDashboard',
  data() {
    return {
      imgUrl: process.env.imgUrl,
      diseases: [],

      // Options
      query: {
        type: '',
        search: null,
        limit: 10,
        page: 1,
      },

      modal: false,
      modalType: null,

      // Edit Disease
      disease_id: null,
    }
  },
  async fetch() {
    this.disease_id = null

    const query = {}

    for (const [key, value] of Object.entries(this.query)) {
      if (value) {
        query[key] = value
      }
    }

    const resDiseases = await this.$axios.get('/api/disease', {
      params: query,
    })

    this.diseases = resDiseases.data.result
    this.query.page = this.diseases.current_page
  },
  computed: {
    // Get disease list
    listDiseases() {
      return this.diseases.data
    },

    // Get page list
    listPages() {
      return this.diseases.links
    },
  },
  methods: {
    // Change Page
    changePage(page) {
      this.query.page = page
      this.$fetch()
    },

    // Previous Page
    previousPage() {
      if (this.query.page != 1) {
        this.query.page = this.query.page - 1
        this.$fetch()
      }
    },

    // Next Page
    nextPage() {
      if (this.query.page != this.diseases.last_page) {
        this.query.page = this.query.page + 1
        this.$fetch()
      }
    },

    // Open Modal
    showModal(modalType, id) {
      this.modal = true
      this.modalType = modalType
      this.disease_id = id
    },

    // Close Modal
    closeModal() {
      this.modal = false
      this.modalType = null
      this.disease_id = null
    },
  },
}
</script>
